...
4. 길이 + 형식 (A4 1페이지 — 엄격):
   ...
   overview    list[str], 2~3개 불릿. 각 35~55자.
               ...
               좋은 예: "캠페인 대상: 하이엔드 수입 초콜릿 카테고리"
                       "캠페인 기간: 2026.01.18 ~ 02.14 (27일)"
                       "목표: 고가치 소비자 정밀 도달 + 구매 전환"
   background  list[str], 2~3개 불릿. 각 35~60자.
               마주한 과제·시장 환경 사실 단문.
   strategy    **str (서술형, 컴팩트)**. 2~3문장, 한국어 100~180자.
               타게팅 / 미디어 / 빈도 / 측정 접근의 **큰 그림** 을 한 호흡으로.
               **세부 수치는 하나하나 읊지 말 것** — 그건 차트와 인사이트 callout
               의 몫. 여기서는 전략 자체의 사고 흐름과 핵심 한두 숫자만.
               좋은 예: "하이엔드 초콜릿 구매 이력·관심 기반 고가치 타겟 세그먼트에
                        집중 도달하여 발렌타인 시즌 구매 전환을 견인했습니다.
                        단순 도달보다 구매력 높은 소비자 선별이 효율의 핵심이었으며,
                        퍼널 전 단계에서 광고 노출자의 기여도가 점진 상승하는
                        구조가 확인되었습니다."
               나쁜 예: 불릿 4개로 수치 나열 ("성장률 +358%, 카트 +557% ..." 식)
   insights    list[str], 정확히 3개. 각 1~2문장 (60~95자).
               "○ 한 줄 학습" 형태. **세부 수치 변화 해설은 여기에 모음.**
...
NARRATIVE_SCHEMA = {
    ...
    "properties": {
        ...
        "strategy":   {"type": "string"},          # 03 은 서술형 (단락)
...
# 불릿 리스트로 렌더할 섹션. strategy 는 의도적으로 빠져있음:
# 03 은 짧고 컴팩트한 서술형 (큰 그림 위주, 세부 수치는 인사이트 callout 에서).
BULLET_SECTIONS = {"overview", "background"}
