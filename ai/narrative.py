...
4. 불릿 사용 원칙 (★ 매우 중요 — 모든 list 필드 공통):
   - 불릿(▪)은 **동일 층위의 병렬 메시지가 2개 이상** 일 때만 사용합니다.
   - 단일 메시지(섹션에 할 말이 본질적으로 한 덩어리) 면 list 에 항목 1개만
     담아 응답하세요. 템플릿이 자동으로 단락(<p>) 으로 렌더합니다.
   - 억지로 한 메시지를 쪼개서 불릿 2개 만들지 마세요. "도입 한 줄 + 그 부연 한
     줄" 같은 구조는 한 단락 한 항목이지 두 불릿이 아닙니다.
   - 좋은 예 (overview, 단일 메시지):
       ["광고주는 카테고리 최성수기 진입을 앞두고 단순 노출이 아닌 시장 점유율
         확장을 핵심 목표로 설정했습니다."]
       → 1개 항목 → <p> 단락으로 렌더
   - 좋은 예 (background, 병렬 한계 2가지):
       ["경쟁 마케팅이 집중되는 단기 구간, 광고 물량 확대만으로 차별화 어려움",
        "광고 노출과 실제 구매 간 연결을 검증할 측정 기반 부재"]
       → 2개 항목 → ▪ 불릿으로 렌더
   - 나쁜 예: 단일 메시지를 두 문장으로 쪼개 불릿 2개로 응답 (시각적 노이즈).
...
6. 길이 + 형식 (A4 1페이지 — 엄격):
   ...
   overview    list[str], **1~3개 항목**. 위 [불릿 사용 원칙] 적용:
               할 말이 한 덩어리면 항목 1개로 (긴 한 문장 OK, 80~120자 가능),
               병렬 메시지 2~3개면 각 35~60자 단문으로.
               광고주가 직면한 시장·KPI·목표 (모티브 관점에서 정의).
               좋은 예 (단일): ["광고주는 카테고리 최성수기 진입을 앞두고 단순
                                노출이 아닌 시장 점유율 확장을 핵심 목표로
                                설정했습니다."]
               좋은 예 (병렬): ["대상: 하이엔드 카테고리",
                                "기간: 시즌 최성수기 1개월",
                                "목표: 단기 시장 점유율 확장"]
   background  list[str], **1~3개 항목**. 위 [불릿 사용 원칙] 적용.
               광고주가 마주한 한계·과제 (모티브가 어떤 문제를 해결할지 도입부).
               병렬 한계 2개 이상이면 불릿, 단일 한계면 항목 1개.
               좋은 예 (병렬): ["경쟁 마케팅이 집중되는 단기 구간, 광고 물량
                                확대만으로 차별화 어려움",
                                "광고 노출과 실제 구매 간 연결을 검증할 측정
                                 기반 부재"]
   strategy    list[str], **2~4개 항목** (lead-in 포함).
               - 첫 항목 (lead-in): 모티브가 적용한 솔루션 한 문장. 항상 포함.
                 예: "모티브인텔리전스는 시즌 수요 곡선에 맞춘 정밀 타게팅
                      솔루션을 적용했습니다."
               - 나머지 1~3개: 모티브가 한 구체 액션. 각 40~80자.
                 타게팅 / 미디어 운영 / 측정 접근 — 한 항목 한 액션.
                 액션이 본질적으로 1개면 lead + action 한 짝 (총 2개),
                 액션 2개 이상이면 lead + bullets (총 3~4개).
