...
4. 불릿 사용 원칙 (★ 매우 중요 — 모든 list 필드 공통):
   - **기본 = 단락(paragraph)**. 항목 텍스트를 평범하게 적으면 단락으로 렌더됩니다.
   - **불릿이 필요할 때만** 항목 맨 앞에 `- ` 접두사를 붙이세요.
     접두사가 없는 항목은 단락, 있는 항목은 ▪ 불릿입니다.
   - 단락과 불릿은 한 섹션 안에서 자유롭게 섞을 수 있습니다. 연속된 불릿은
     자동으로 하나의 리스트로 묶입니다.

   언제 불릿을 쓰나:
     - 동일 층위 사실(facts)을 짧게 병렬 나열할 때만.
     - 한 메시지를 풀어 쓴 문장 흐름은 단락. "한 호흡으로 읽혀야 자연스러운가?"
       기준으로 판단.

   좋은 예 (overview, 단락 위주):
     [
       "광고주는 카테고리 최성수기 진입을 앞두고 단순 노출 확보가 아닌 시장
        점유율 확장을 핵심 목표로 설정했습니다.",
       "고관여 구매자에게 정밀 도달하면서 시즌 단기간 내 입지를 강화하는 것이
        과제였습니다."
     ]
     → 두 단락 흐름. 불릿 X.

   좋은 예 (background, 단락 + 병렬 한계 불릿 묶음):
     [
       "시즌 성수기는 카테고리 경쟁이 가장 치열한 시기로, 광고 물량 확대만으로는
        차별화가 어렵습니다. 브랜드는 두 가지 한계에 직면해 있었습니다.",
       "- 경쟁사 대비 메시지 노출 점유 확보가 어려운 단기 집중 구간",
       "- 광고 노출과 실제 구매 간 연결을 검증할 측정 기반 부재"
     ]
     → 도입 단락 1 + 불릿 2개 자동 그룹화.

   좋은 예 (strategy, lead 단락 + 액션 불릿):
     [
       "모티브인텔리전스는 시즌 수요 곡선에 맞춘 정밀 타게팅 솔루션을
        적용했습니다.",
       "- 고소득자 비중이 높고 제품 관심도가 높은 수도권·부산 거주 인구 중 시즌
         구매 경향이 높은 연령대를 정밀 세그먼트로 추출",
       "- 시즌 수요 곡선에 맞춰 CTV 노출량을 스케일업하고, 프리미엄 대화면 가구에
         집중 도달",
       "- 오프라인·온라인 결제 데이터를 매칭한 풀퍼널 측정으로 실제 구매 전환까지
         정량 검증"
     ]

   나쁜 예: 모든 항목에 `- ` 붙여서 줄글 흐름을 불릿 도배로 만드는 것.
...
6. 길이 + 형식 (A4 1페이지 — 엄격):
   ...
   overview    list[str], **1~4개 항목**. 위 [불릿 사용 원칙] 적용.
               기본은 단락(불릿 X). 광고주 KPI/목표 한 흐름이면 1~2 단락,
               대상·기간·목표 같은 사실 나열이면 도입 단락 1 + `- ` 불릿 2~3.
   background  list[str], **1~4개 항목**. 위 [불릿 사용 원칙] 적용.
               기본은 단락. 한계가 병렬 2~3가지면 도입 단락 1 + `- ` 불릿 2~3.
   strategy    list[str], **2~5개 항목**. 위 [불릿 사용 원칙] 적용.
               - 첫 항목: 모티브가 적용한 솔루션 한 문장. 단락(불릿 X). 항상 포함.
                 예: "모티브인텔리전스는 시즌 수요 곡선에 맞춘 정밀 타게팅
                      솔루션을 적용했습니다."
               - 나머지 1~4개: 모티브가 한 구체 액션. 각 40~80자.
                 액션이 2개 이상이면 모두 `- ` 접두사로 불릿 처리.
                 액션이 1개뿐이면 그냥 단락으로 (`- ` 없이) 추가.
...
7. 출력은 지정된 JSON 스키마만. 설명·머리말·코드펜스 금지.
"""
